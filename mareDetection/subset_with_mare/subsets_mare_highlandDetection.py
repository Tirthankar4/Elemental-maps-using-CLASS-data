...
def parse_and_generate_subsets(figs_and_axs, csv_file, csv_subset_files, mare_subset_files, highland_subset_files=[]):
    ...
    mare_subsets = {mare: {element: [] for element in elements} for mare in mareList}
            ...
            for element in elements:
                if not found_elements[element]:  # Only process if the element has not been found
                    ...
                    if row['Si_detection'] == True and row[detection_col] == True:
                        ...
                        mareName = gridObject.mareAtLocation(row['BORE_LAT'], row['BORE_LON'])
    ...
    mare_subsets_dfs = {mare: {element: pd.DataFrame(mare_subsets[mare][element]) for element in elements} for mare in mareList}
...
gridObject=MakeGrid("mare_boolean.txt", "mare_name.txt")
...
mare_subset_files = dict([(element, f"/home/kaustav_b_ph.iitr/Plotting/Subsets/{mare}_{element}_subset.csv") for mare in mareList for element in elements_])
...
for csv_file in csv_files:
    try:
        print(csv_file)
        parse_and_generate_subsets(figs_and_axs, csv_file, csv_subset_files, mare_subset_files) #highland_subset_files
